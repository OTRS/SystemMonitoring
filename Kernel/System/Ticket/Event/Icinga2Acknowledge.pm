...
sub _HTTP {
    ...
    if ( $Ticket{ $Self->{Fservice} } !~ /^host$/i ) {
        my $Filter=sprintf('match("%s",host.name)&&match("%s",service.name)',
                            $Ticket{$Self->{Fhost}},
                            $Ticket{$Self->{Fservice}});
        $URL .= sprintf("?type=Service&filter=%s",URI::Escape::uri_escape($Filter));
    }
    else {
        my $Filter=sprintf('match("%s",host.name)',
                            $Ticket{$Self->{Fhost}});
        $URL .= sprintf("?type=Host&filter=%s",URI::Escape::uri_escape($Filter));
    }

    if(length($Author) == 0) {
        my @UserFullname=split(/ /,$UserData{UserFirstname}.' '.$UserData{UserLastname});
        $Author = sprintf("%s %s",$UserFullname[0],($#UserFullname > 0?' '.$UserFullname[$#UserFullname]:''));
    }
    ...
    if ( !$Response->is_success() ) {
        ...
        if($ResponseString) {
                my $ResponseJSON = $JSONObject->Decode(Data=>$ResponseString);
                if(!IsHashRefWithData($ResponseJSON)) {
                    $Kernel::OM->Get('Kernel::System::Log')->Log(
                        Priority => 'error',
                        Message  => sprintf("Error sending ack to Icinga2. Fail to decode json (http %s): %s",$Response->status_line(),$ResponseString),
                    );
                    return;
                }
                my $Results = $ResponseJSON->{results};
                if($ResponseJSON && IsArrayRefWithData($Results) && IsHashRefWithData($Results->[0]) && $Results->[0]->{status} && $Results->[0]->{code} > 299) {
                    $Kernel::OM->Get('Kernel::System::Log')->Log(
                        Priority => 'error',
                        Message  => sprintf("Error sending ack to Icinga2: %s (%s)",$Response->status_line(), $Results->[0]->{status}),
                    );
                }
        }
    }
